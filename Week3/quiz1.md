...
- 답: Create a merge commit은 두 브랜치를 공통 부모롤 하는 새로운 commit을 만들어 커밋들이 그대로 병합되는 것, Sqush and merge는 커밋들을 하나의 커밋으로 만들어 병합시키는 것, Rebase and merge는 커밋들의 base를 재설정하는것이다.
...
- 답: 가장 최근에 develop이라는 브랜치에 docs: readme 추가 라는 커밋을 했고 그 커밋의 id는 a1s2d3f이다.
...
- 답: revert. reset은 commit을 공유하는 다른 브랜치에도 영향을 줄 수 있어 위험하다.
